...
{
    uint8_t system_cycle_time;    // 1–30 秒，0 為哨兵值（見 §3）
...
| 欄位 | 大小 | 說明 |
|------|------|------|
| `system_cycle_time` | 1 | Hook alarm window（秒）。值範圍 1–30，超過或為 0 會被 Hook 端 clamp。**值 0 在 v3 起賦予新語意（哨兵）**，見 §3 |
...
### 3.2 Flag-only write（`system_cycle_time = 0` 哨兵）

`system_cycle_time` 原本範圍是 1–30；當值為 **0** 時 Hook v3 將其視為「flag-only write」哨兵：

- Hook **不會** 把 17 byte 寫進 settings flash（XYZ 軸與 system_cycle_time 全部忽略）
...
Belt v_new 韌體在「看到任一 SAFE-role beacon 為 fresh」期間，會 **自動** 透過本機 BLE 對所有已連線的 Hook slot 每 3 秒送一次 flag-only write（`system_cycle_time=0`、`flags=0x80`），維持 Hook 端的 grace window：
...
reserve:                0x0000
hook_settings:
  system_cycle_time:    0x00            ← 哨兵
  flags:                0x80            ← FLAG_IN_SAFE_ZONE
  x / y / z:            (忽略，建議全填 0)
```

Belt 收到後會比對 ID、轉送這個 17-byte block 到對應 Hook 的 BLE RX；Hook 看到 `system_cycle_time=0` 會走 flag-only path，不更動 settings、僅執行 reset counter。
...
hook[0] (25B): DC 06 75 F8 2C C8 00 00 00 80 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
                └─── id ──────┘ └rsv┘ │  │
                                       ...
                                       └─── system_cycle_time = 0（flag-only 哨兵）
...
| `system_cycle_time = 0` | 直接轉送 17 bytes | 觸發 flag-only path，不 persist settings |
| `system_cycle_time > 30` | 直接轉送 | clamp 回 `DEFAULT_HOOK_CYCLE_TIME / 1000`（10 秒） |
...
| v3 | v_new | **舊版** | **不安全** — 舊 Hook 看到 `system_cycle_time=0` 不會走 flag-only path，會把 settings 全打成 0（XYZ sensitivity 都 0 → 偵測失效）。必須先把 Hook 升到 v_new |
